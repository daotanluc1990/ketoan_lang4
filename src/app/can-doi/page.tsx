...
export default async function CanDoiPage({ searchParams }: { searchParams?: PageSearchParams }) {
  ...
  return (
    <div className="space-y-2.5">
      ...
      {limitationCount ? <section className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-1.5 text-xs font-bold text-amber-900">Còn {limitationCount} điểm cần kế toán rà trước khi chốt.</section> : null}
      ...
      <section className="grid gap-2 xl:grid-cols-[minmax(0,2fr)_minmax(260px,0.8fr)]">
        <Card>
          ...
          <div className="mt-2"><ReportTable headers={['Nhóm', 'Chỉ số', 'Số tiền', 'Tuần trước', 'Chênh lệch', 'Trạng thái', 'Ghi chú']} rows={report.balanceRows} maxHeight="max-h-[390px]" /></div>
        </Card>
        <Card>
          <CardTitle>Cảnh báo cần xem</CardTitle>
          <div className="mt-2"><ReportTable headers={['Mục', 'Giá trị', 'Hành động']} rows={[["Tồn âm", `${report.totals.negativeStockCount} mặt hàng`, "Rà nhập/xuất/kiểm kê"], ["Nguồn thiếu", report.missingSources.length ? report.missingSources.join(', ') : 'Không thiếu', "Bổ sung trước khi chốt"], ["Chốt báo cáo", hasBalanceData ? "Có dữ liệu" : "Chưa đủ", "Đối chiếu trước khi gửi"]]} maxHeight="max-h-[220px]" /></div>
        </Card>
      </section>
      <Card>
        <CardTitle>Giới hạn cần kế toán rà</CardTitle>
        <div className="mt-2"><ReportTable headers={['Nhóm', 'Vấn đề', 'Ảnh hưởng', 'Hành động']} rows={report.financeLimitationRows} maxHeight="max-h-[260px]" /></div>
      </Card>
    </div>
  );
}
