...
export default async function ImportNhapLieuPage() {
  ...
  return (
    <div className="space-y-2.5">
      ...
      <Card><CardTitle>Batch upload nhiều file</CardTitle><div className="mt-2"><BatchUploadMock /></div></Card>
      <section className="grid gap-2 xl:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
        <Card><CardTitle>Quy tắc import production</CardTitle><ul className="mt-2 grid gap-1.5 text-xs font-semibold text-slate-700"><li>• Upload chỉ đọc, không ghi ngay.</li><li>• Dry-run preview trước khi xác nhận.</li><li>• File lỗi hoặc dữ liệu lệch không được ghi.</li><li>• Mọi lần import phải ghi lịch sử và audit.</li><li>• Sheet trống thì báo chưa đủ dữ liệu.</li></ul></Card>
        <Card><CardTitle>Lịch sử import gần nhất</CardTitle><div className="mt-2"><ReportTable headers={['Ngày import', 'Người import', 'Trạng thái', 'Ghi chú', 'Dòng mới', 'Dòng lỗi']} rows={historyRows.length ? historyRows : [['—', '—', 'Chưa đủ dữ liệu', 'Chưa có lần import thật', '0', '0']]} maxHeight="max-h-[360px]" /></div></Card>
      </section>
    </div>
  );
}
