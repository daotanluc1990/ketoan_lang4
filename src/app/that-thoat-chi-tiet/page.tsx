    ...
    <div className="space-y-2.5">
      ...
      <section className="grid gap-2 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)]">
        ...
        <Card>
          ...
          <div className="mt-2"><ReportTable headers={['NVL', 'ĐVT', 'Chênh SL', 'Giá trị', 'Tỷ lệ', 'Trạng thái', 'Hành động']} rows={report.lossTop5Rows.map((row) => [row[0], row[1], row[2], row[3], row[4], row[7], row[8]])} maxHeight="max-h-[300px]" /></div>
        </Card>
      </section>
      <section className="grid gap-2 xl:grid-cols-2">
        <Card>
          <CardTitle>Hao hụt / vượt định mức chế biến</CardTitle>
          <div className="mt-2"><ReportTable headers={['Món/NVL', 'Định mức', 'Thực tế', 'Vượt SL', 'Tỷ lệ', 'Hành động']} rows={report.lossTop5Rows.map((row) => [row[0], row[5] ?? 'Định mức', row[2], row[2], row[4], 'Rà thao tác/định lượng'])} maxHeight="max-h-[260px]" /></div>
        </Card>
        <Card>
          <CardTitle>Thất thoát tồn kho</CardTitle>
          <div className="mt-2"><ReportTable headers={['NVL', 'Tồn lệch', 'Giá trị', 'Tỷ lệ', 'Nguyên nhân', 'Hành động']} rows={report.lossTop5Rows.map((row) => [row[0], row[2], row[3], row[4], row[6] ?? 'Cần kiểm', row[8]])} maxHeight="max-h-[260px]" /></div>
        </Card>
      </section>
    </div>
  );
