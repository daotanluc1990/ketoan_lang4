...
export default async function ThatThoatChiTietPage({ searchParams }: { searchParams?: PageSearchParams }) {
  ...
  return (
    <div className="space-y-3">
      <PageHeader title="Báo cáo thất thoát chi tiết" description="Xem NVL thiếu, dư, giá trị lệch và hành động xử lý." status={hasLoss ? 'Cảnh báo' : 'Chưa đủ dữ liệu'} />
      ...
      <section className="grid gap-2 md:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard label="Số NVL theo dõi" value={`${report.sourceCounts.lossRows}`} status={hasLoss ? 'good' : 'neutral'} trend="Đã có dữ liệu" compact />
      ...
      <Card>
        <CardTitle>Top 5 nguyên liệu cần xử lý</CardTitle>
        <div className="mt-2"><ReportTable headers={['NVL', 'ĐVT', 'Chênh SL', 'Giá trị lệch', 'Tỷ lệ', 'Trạng thái', 'Hành động']} rows={report.lossTop5Rows.map((row) => [row[0], row[1], row[2], row[3], row[4], row[7], row[8]])} maxHeight="max-h-[320px]" /></div>
      </Card>
    </div>
  );
}
