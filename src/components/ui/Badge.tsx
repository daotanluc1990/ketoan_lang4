...
export function Badge({ children, variant = 'neutral' }: { children: React.ReactNode; variant?: keyof typeof variants }) {
  return <span className={clsx('inline-flex h-5 items-center whitespace-nowrap rounded-full px-2 text-[10px] font-extrabold leading-none', variants[variant])}>{children}</span>;
}
